...
const Odds = () => {
  const getInitialLeague = () => {
    const storedLeague = localStorage.getItem('selectedOddsLeague');
    if (storedLeague) {
      return { id: storedLeague };
    }
    return { id: '1' }; // Default to NFL
  };

  const [selectedOddsLeagueId, setSelectedOddsLeagueId] = useState(getInitialLeague().id);
  ...
  const selectedOddsLeague = useMemo(() => ({ id: selectedOddsLeagueId }), [selectedOddsLeagueId]);
  ...
  const fetchOddsData = async (leagueId) => {
    setIsLoading(true);
    setNoGamesMessage('');
    setOddsData([]);
    setBestOddsData({});
    ...
    console.log(`Fetching odds data for league: ${leagueId}`);
  
    try {
      const response = await axios.get(`https://api.opensourcesports.xyz/odds?league_id=${leagueId}`);
      console.log(`Odds data fetched for league: ${leagueId}`, response.data);
  ...
  useEffect(() => {
    fetchOddsData(selectedOddsLeagueId);
    // eslint-disable-next-line
  }, [selectedOddsLeagueId]);
  
  const formatDateTime = (dateTimeString, isLive) => {
    if (isLive) {
      return <span className="live-text">Live</span>;
    }
      ...
      <div className="form-group">
        ...
        <div className="checkbox-group">
          <label>
            <input
              type="checkbox"
              checked={selectedOddsLeague.id === '1'}
          ...
          <label>
            <input
              type="checkbox"
              checked={selectedOddsLeague.id === '3'}
          ...
          <label>
            <input
              type="checkbox"
              checked={selectedOddsLeague.id === '6'}
          ...
          <label>
            <input
              type="checkbox"
              checked={selectedOddsLeague.id === '7'}
              ...
              <div key={game.id}>
                ...
                >
                  ...
                  <div className="game-info">
                    ...
                    {formatDateTime(game.commence_time, game.is_live)}
                  </div>
                  <div className="team-info">
                    <img
